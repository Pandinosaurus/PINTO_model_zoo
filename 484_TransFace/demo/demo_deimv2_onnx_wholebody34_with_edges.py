...
def build_head_crop_base_name(
    *,
    ...
    if file_path is not None and images_dir is not None:
        relative_path = os.path.relpath(file_path, start=images_dir)
        relative_stem = Path(relative_path).with_suffix('')
        return '__'.join(relative_stem.parts)

    if video is not None and is_parsable_to_int(video):
        video_stem = f'camera_{video}'
    else:
...
def build_ijb_landmark_lines(
    *,
    ...
    faces = [box for box in boxes if box.classid == 16]
    ...
    for face in faces:
        ...
        left_mouth_x = float(mouth.x1)
        left_mouth_y = float(mouth.cy)
        right_mouth_x = float(mouth.x2)
        right_mouth_y = float(mouth.cy)

        ijb_lines.append(
            f'{relative_image_path} '
            f'{float(left_eye.cx):.6f} {float(left_eye.cy):.6f} '
            f'{float(right_eye.cx):.6f} {float(right_eye.cy):.6f} '
...
def main():
    ...
    parser.add_argument(
        '-fco',
        '--face_crop_output_dir',
        type=str,
        default=None,
        help='Directory to save classid=16 (Face) crop images as flat PNG files.',
    )
    parser.add_argument(
        '--output_ijb_landmarks_txt',
        type=str,
        default=None,
        help='Output path for aggregated IJB-format 5-point landmark TXT.',
    )
    parser.add_argument(
        '-bblw',
    ...
    face_crop_output_dir: Optional[str] = args.face_crop_output_dir
    output_ijb_landmarks_txt: Optional[str] = args.output_ijb_landmarks_txt
    ...
    ijb_landmarks_output_path: Optional[str] = output_ijb_landmarks_txt
    if ijb_landmarks_output_path is None and face_crop_output_dir is not None:
        ijb_landmarks_output_path = os.path.join(face_crop_output_dir, 'ijb_landmarks.txt')
    if ijb_landmarks_output_path is not None and images_dir is None:
        print(Color.RED('ERROR: IJB landmark TXT output is supported only with --images_dir.'))
        sys.exit(0)
    ...
    while True:
        ...
        if face_crop_output_dir is not None:
            face_crop_records = build_face_crop_records(
                ...
                file_path=file_paths[file_paths_count] if file_paths is not None else None,
                images_dir=images_dir,
                video=video,
                movie_frame_count=movie_frame_count,
            )
    ...
    if ijb_landmarks_output_path is not None:
        output_dir = os.path.dirname(ijb_landmarks_output_path)
        if output_dir:
            os.makedirs(output_dir, exist_ok=True)
        with open(ijb_landmarks_output_path, 'w') as f:
            for line in ijb_landmark_lines:
                f.write(f'{line}\n')
