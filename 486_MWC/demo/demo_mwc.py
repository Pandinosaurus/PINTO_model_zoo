...
MASKED_LABEL = 'Masked'
MASKED_COLOR = (0, 210, 0)  # readable green for masked label/bounding boxes
...
class Box():
    ...
    body_label: str = ''
    head_prob_masked: float = -1.0
    head_state: int = -1  # -1: Unknown, 0: not_masked, 1: masked
    head_label: str = ''
...
class MWC(AbstractModel):
    def __init__(
        self,
        *,
        runtime: Optional[str] = 'onnx',
        model_path: Optional[str] = 'mwc_l_48x48.onnx',
        providers: Optional[List] = None,
    ):
        super().__init__(
            runtime=runtime,
    ...
    def __call__(self, image: np.ndarray) -> float:
        if image is None or image.size == 0:
            raise ValueError('Input image for MWC is empty.')
        ...
        prob_masked = float(np.squeeze(outputs[0]))
        return float(np.clip(prob_masked, 0.0, 1.0))
    ...
    def _preprocess(
        ...
        if height <= 0 or width <= 0:
            raise ValueError('Invalid target size for MWC preprocessing.')
...
def main():
    ...
    parser.add_argument(
        ...
        '--mwc_model',
        type=str,
        default='mwc_l_48x48.onnx',
        help='ONNX file path for the MWC masked classifier.',
    )
    ...
    mwc_model_file: str = args.mwc_model
    ...
    mwc_classifier = MWC(
        runtime='onnx',
        model_path=mwc_model_file,
        providers=providers,
    )
    use_head_crops = Path(mwc_model_file).name == 'mwc_l_48x48.onnx'
    ...
    while True:
        ...
        for box in target_boxes:
            ...
            try:
                prob_masked = mwc_classifier(image=rgb_crop)
            except Exception:
                continue
            box.head_prob_masked = prob_masked
        ...
        for box in boxes:
            ...
            masked_label_active = classid in (0, 7) and bool(box.head_label)
            if classid in (0, 7):
                if box.head_label or (box.head_prob_masked is not None and box.head_prob_masked >= 0.0):
                    attr_txt = f'{box.head_label} {box.head_prob_masked:.3f}' if box.head_label else f'{box.head_prob_masked:.3f}'
                else:
                    attr_txt = ''

            attr_color = MASKED_COLOR if masked_label_active else color
            if attr_txt == '':
                continue
            cv2.putText(
                debug_image,
